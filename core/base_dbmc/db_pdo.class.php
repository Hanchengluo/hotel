...
// DB	
	...
	function init_db($is_master_db=true, $dbname=false)
	{
		...
		if($is_master_db)
		{
			...
			{
				$db_base_obj_master = new PDO($dsn,$user,$passwd);
				$db_base_last_master_time = time();//更新主库连接时间
				
				if (strlen($db_config_slave['charset']) > 0)
				{
					$db_base_obj_master->exec("SET NAMES '{$db_config_slave['charset']}' ");
				}
		...
		else
		{
			...
			{
				$db_base_obj_slave = new PDO($dsn,$user,$passwd);
				$db_base_last_slave_time = time(); //更新辅库连接时间
				
				if (strlen($db_config_slave['charset']) > 0)
				{
					$db_base_obj_slave->exec("SET NAMES '{$db_config_slave['charset']}' ");
				}
