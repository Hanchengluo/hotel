...
// DB	
	...
	{
		global $db_config_master, $db_config_slave,$db_base_obj_master,$db_base_obj_slave,$db_base_last_master_time,$db_base_last_slave_time;
		...
		else							//is slave
		{
			...
			$host = $db_config_slave['host'];
			$port = $db_config_slave['$port'];
		...
		if($is_master_db)
		{
			if(!$db_base_obj_master || ( time()- $db_base_last_master_time > $this->wait_timeout) )
			{
				$db_base_obj_master = mysql_connect($host,$user,$passwd);
				$db_base_last_master_time = time();//更新主库连接时间
				mysql_select_db("$dbname");
				if (strlen($db_config_slave['charset']) > 0)
				{
					mysql_query("SET NAMES '{$db_config_slave['charset']}' ",$db_base_obj_master);
			...
			$this->db_obj = &$db_base_obj_master;
		}
		else
		{
			
			if(!$db_base_obj_slave || (time()- $db_base_last_slave_time > $this->wait_timeout) )
			{
				$db_base_obj_slave = mysql_connect($host,$user,$passwd);
				$db_base_last_slave_time = time(); //更新辅库连接时间
				mysql_select_db("$dbname");
				if (strlen($db_config_slave['charset']) > 0)
				{
					mysql_query("SET NAMES '{$db_config_slave['charset']}' ",$db_base_obj_slave);
				}
	...
	function insert($tblname, $arr_insert_data, $dbname = false, $is_replace = false)
	{
		...
		// 执行
		$res=mysql_query($sql,$this->db_obj);
	...
	function select($sql, $is_ok = false, $is_master_db=false, $dbname = false)
	{
		...
		$query =mysql_query($sql,$this->db_obj);
	...
	function get_alllist($tblname, $pageid, $pagelen, $order="", $where="", $dbname = false, $field=false)
	{
		global $program_sql_call_time;
		$time_start = microtime_float();
		...
		{
			$sql ="select {$field} from {$tblname} {$where} {$order}" . $sqllimit;
		}
