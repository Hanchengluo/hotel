...
(function () {
    var _S = function (obj) {
        ...
        if (typeof (obj) == 'string') {
            ...
            if (that.e == null && typeof window.console != 'undefined')
                console.info("[Warm] element " + obj + " is not exist!!!");
        } else {
            that.e = obj;
        }
    }

    window.$ = window.Sbory = function (obj) {
        return new _S(obj)
    }
...
if ($.browser == 'WeiQiao') {
    ...
    function addButtonEvent() {
        ...
        document.addEventListener("menubutton", function () {//菜单键要通过添加监听才能够捕获
            ...
            if (ret)
                ret = $.plugs.keyDefaultProcess(tmpObj) || 0;
            return ret;
        });
    }
}
...
$.dg = function () {
    ...
    switch ($.browser) {
        case 'Coship':
            $.each(show, function (v) {
                Utility.println('sbory_debug==== ' + v);
            });
            break;
        case 'iPanel':
        case 'Sunniwell':
            $.each(show, function (v) {
                iPanel.debug('sbory_debug==== ' + v);
            });
            break;
        case 'Android':
        case 'Chrome':
        case 'WeiQiao':
            $.each(show, function (v) {
                console.log('sbory_debug==== ' + v);
            });
            break;
        case 'Hisu':
            $.each(show, function (v) {
                System.debug('sbory_debug==== ' + v);
            });
            break;
        case 'OTTSW':
            $.each(show, function (v) {
                SWSystem.log('sbory_debug==== ' + v);
            });
            break;
        default:
            break;
...
$.eval = function (o) {
    ...
    if (o.indexOf("`") > -1)
        o = $.replaceAll(o, "`", "'");
    o = o.replace(/[\r]?\n/g, '');
    return (o.length > 0 && o.indexOf('<html><body></body></html>') < 0) ? eval('(' + o + ')') : '';
}
//json格式转成xml格式
...
$.getFocus = function (varsArr, focusStr) {
    ...
    if (focusStr != null) {
        ...
        focusArr = $.each(focusArr, function (v) {
            return v - 0
        });
...
$.date = {
    ...
    //传入日期字串，返回日期对象。日期字串格式：2011-11-22 12:34:56或2011-11-22
    getDate: function (source, offset) {
        offset = offset || 0;
        if ('string' == typeof (source)) {
            ...
            return new Date(dateArr[0], parseInt(dateArr[1], 10) - 1, parseInt(dateArr[2], 10) + offset, dateArr[3], dateArr[4], dateArr[5]);
        } else {
            source = source || this.currentDate || new Date();
            ...
            return new Date(year, month, (date2 + offset), hours, minutes, seconds);
        }
    },
...
(function ($) {
    ...
    if (browser != 'iPanel' && typeof iPanel == 'undefined')
        window.iPanel = {eventFrame: {}, debug: function () {}};
    //按键映射
    ...
    switch (browser) {
        ...
        case 'Coship':
            doc.onkeypress = function (event) {
                ...
                if (ret == 0 && event && event.preventDefault)
    ...
    plugs.list.prototype = {
            ...
            if (focusDiv != null) {
                ...
                if (focusDiv._constructor == 'slip') {
                    ...
                    if (focusStyle != 0) {
                        ...
                        if (focusStyle == 1) {
                            if (that[p_focusMoveTime] >= 100)
                                that[p_focusMoveTime] = 30;
        ...
        showList: function () {
            ...
            for (; i < showLen; i++) {
                if (showLoop) {
                    ...
                    if ((i + startPos) < dataLen)
                        that[f_haveData](i, i + startPos);
                    else
                        that.noData(i, i + startPos);
                }
            }
        ...
        focus: function () {
            ...
            if (focusStyle != 1) { //焦点非slip对象
                focusDiv.show();
                if (focusStyle == 2 && (browser == 'iPanel' || browser == 'Chrome')) {
                    focusDiv.webkit('0ms');
        ...
        blur: function () {
            ...
            var focusDiv = that.focusDiv;
            if (focusDiv != null) {
                if (that.focusStyle == 1)
                    focusDiv.moveObj.hide();
                else
                    focusDiv.hide();
            }
      },
        ...
        isLimit: function (source) {
            ...
            if (that.showType == '*') {
                ...
                switch (source) {
                    ...
                    case 'right':
                    case 'r':
                        if (focusPos % cols == cols - 1 || dataPos == dataLen - 1)
                            return true;
                        break;
                    case 'bottom':
                    case 'b':
                        var M_F = Math.floor;
                        if (focusPos >= cols * (that.showRows - 1) || M_F(dataPos / cols) == M_F((dataLen - 1) / cols))
                            return true;
                        break;
    ...
    plugs.slip.prototype = {
        init: function (data) { //id,moveType,moveStep,moveTime
            ...
            $.each(p_keys, function (v) {
                that[v] = data[v] || that[v];
            });
    ...
    plugs.scroll.prototype = {
        init: function (initData) {
            ...
            if (scrollObj == null || bgObj == null || that[p_total] < 2)
                return;
        ...
        scroll: function (pos) {
            ...
            if (scrollType == 1) {
                ...
                if (scrollType == 2 && (browser == 'iPanel' || browser == 'Chrome')) {
        ...
        show: function () {
            ...
            if (that.scrollType != 1)
                scrollObj.webkit(that.moveTime + 'ms').show();
            else
                scrollObj.moveObj.show();
        ...
        hide: function () {
            var that = this,
                    scrollObj = that.scrollObj;
            if (that.scrollType != 1)
                scrollObj.webkit(0).hide();
            else
                scrollObj.moveObj.hide();
    ...
    plugs.input.prototype = {
        ...
        backSpace: function () {
            ...
            if (that.imeType == 'letter') {
                if (that.startPos > 0 && cursorPos == that.startPos)
                    that.startPos--;
            }
        ...
        moveCursor: function (num) {
            ...
            if (imeType == 'letter') {
                ...
                if (cursorPos > that.startPos + that.letterLim)
                    that.cursorPos = that.startPos + that.letterLim;
            }
            that.showInput();
        },
    ...
    plugs.page.prototype = {
        ...
        changePage: function (__num) {
            ...
            if (tmpPage < 0 || tmpPage > len - 1)
                return;
            that.currPage = tmpPage + 1;

            var dp = -1 * postions[tmpPage];
            if (that.scrollType == 1)
                contenter.moveStart(contenter.moveObj.top(), dp);
            else
                contenter.top(dp);
...
$.media = {
    ...
    _connectEvent: function () {
        if ($.browser == 'Hisu')
            MediaPlayerIF.signal.connect($.media._hisuEvent);
    },
                ...
                if (browser == 'iPanel') {
                    ...
                    if (url != "") {
                        if (!empty(operator) && operator.indexOf("陕西") > -1) {
                            ...
                            if (new Boolean(MP3.setProperty) == true)
                                MP3.setProperty("beginData", "50");
                            if (!empty(time))
                                MP3.open(url, time);
                            else
                                MP3.open(url, 100);
    ...
    getCaId: function () {
        ...
        switch ($.browser) {
            ...
            case "OTTSW":
                ...
                break;
            case 'WeiQiao':
                caId = $.mem.getVar('WEIQIAO_CAID');//在开机的页面添加了获取代码并且存储到WEIQIAO_CAID里面了
                break;
        }
        if (!$.empty(caId))
            return caId;
        else
            return "90000000000000009";
    },
    getMAC: function () {
        var macAddress = "";
        switch ($.browser) {
            ...
            case "Coship":
                ...
                break;
            case "Chrome":
                macAddress = "ff-ff-ff-ff-ff-ff-ff";
                break;
            case "Hisu":
                var mac_addr = "";
...
//配置信息
var INTERFACE_SERVER = "http://124.47.3.83:8088",
        OPEN_USER = "hotel",
        OPEN_PASSWD = "123456",
...
function initRequestUrl(api, params) {
    ...
    return api = INTERFACE_SERVER + api + "?" + p.join("&");
}

function urlCmdMap(cmdStr) {
    var cmd = cmdStr.toLowerCase().split(":")[1];
    switch (cmd) {
        case 'exittotv':
...
function hotelMsgNotice() {
    ...
    if (pageUrl.indexOf('/static/') < 0 || pageUrl.indexOf('/lang_') < 0 || pageUrl.indexOf('/welcome/') > -1 || pageUrl.indexOf('/hotelMessage') > -1)
        return;
    HOTEL_AUTH_INFO = HOTEL_AUTH_INFO || $.mem.getVar('HISU_SMART_HOTEL_ROOM_INFO');//房间信息，开机认证时获得
    if ($.empty(HOTEL_AUTH_INFO))
        return;
...
function hotelMsgNoticeCallback(__text) {
    ...
    if (!$.empty(tmp) && tmp.dataList && tmp.dataList.length > 0) {
        ...
        $.plugs.dialog.keyProcess = function (keyObj) {
            if (focusArea != $.plugs.dialog.focusArea)
                return 1;
            switch (keyObj.key) {
                case "LEFT":
