...
if ($.browser == 'WeiQiao') {
    ...
    document.addEventListener('deviceready', addButtonEvent, false);
    function addButtonEvent() {
        document.addEventListener("backbutton", function () {//返回键要通过添加监听才能够捕获
            console.log("===========weiQiao==================监听=====back===");
...
//配置信息
var INTERFACE_SERVER = "182.92.156.224:8088",
        OPEN_USER = "hotel",
        OPEN_PASSWD = "123456",
        HOTEL_AUTH_INFO = null,
        HEART_TIME_INTERVAL = 5,
        HOTEL_NOTICE_INTERVAL = 2,
        SMART_AD_SERVER = "123.56.102.119:8086";
    ...
    switch (cmd) {
        case 'exittotv'://基本频道
            if ($.browser == 'iPanel') {//湖南广电的进入直播写法
                ...
                if (c_c_num != null && c_c_num.length > 0)
                    c_index = parseInt(c_c_num);
                else
                    c_index = 1;
...
function hotelUserBehavior() {
    ...
    HOTEL_AUTH_INFO = HOTEL_AUTH_INFO || $.mem.getVar('HISU_SMART_HOTEL_ROOM_INFO');//房间信息，开机认证时获得
    if ($.empty(HOTEL_AUTH_INFO) && params.functionCode != 'welcome')
        return;
