...
/*
 * create table tbl_hotel_program_manager (id int unsigned primary key auto_increment, program_json_address text default null, version int unsigned, create_time TIMESTAMP NULL DEFAULT now());
 */

class db_hotel_program_manager extends db_base {
	
	var $tbl_name = "tbl_hotel_program_manager";

	function db_hotel_program_manager() {
		parent::db_base();
	}
	...
	//修改一条数据
	function admin_update_hotel_program_manager_by_id($id, $program_json_address, $program_version) {
		global $config;
		...
		//详细字段
		$ary = array();
		$ary["program_json_address"] = $program_json_address;
		$ary["version"] = $program_version;
	...
	//添加一条数据
	function admin_insert_hotel_program_manager($program_json_address, $program_version) {
		global $config;
	...
	// 删除一条数据
	function admin_del_hotel_program_manager_by_id($id) {
		global $config;
	...
	function admin_get_hotel_program_manager_by_id($id) {
